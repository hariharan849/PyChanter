...
class DirectoryTree(_QtWidgets.QTreeView):
    folderIcon = _QtGui.QIcon(_constants.folderIcon)
    fileIcon = _QtGui.QIcon(_constants.fileIcon)
    ...
    def _addItemsToTree(self, treeModel, directory, dirItems):
        if dirItems == []:
            ...
            itemFont = _QtGui.QFont("Courier", _constants.defaultDirectoryFontSize)
            noFilesFound.setFont(itemFont)
            treeModel.appendRow([])
        else:
